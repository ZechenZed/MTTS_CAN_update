...
def predict_vitals(video_name, dir_path, data_set, filter):
    ...
    img_cols = 36
    frame_depth = 1
    model_checkpoint = "../my_mtts_last_model.hdf5"
    batch_size = 10
    fs = 25
    ...
    BP_lf = []
    for BP_file in BP_train_path[0:1]:
        temp = np.loadtxt(BP_phase1_path + BP_file)
        temp_lf = []
        for i in range(0, temp.size, 40):
            temp_lf.append(mean(temp[i:i + 40]))
        BP_lf.append(temp_lf)
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        MAE, RMSE, PC = predict_vitals(video_name,dir_path,datatype,True)
