...
def model_train(data_type, device_type, task_num, nb_filters1, nb_filters2,
    ...
    frames = np.load(path + data_type + '_frames_face_large.npy')
    ...
    if data_type == "test":
        model.load_weights(path + 'mtts_sys_kernal99_face_large_drop2_nb256.hdf5')
        model.evaluate(x=(frames[:, :, :, :3], frames[:, :, :, -3:]), y=BP_lf, batch_size=nb_batch)
    else:
        # early_stop = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=5)
        save_best_callback = ModelCheckpoint(filepath=path + 'mtts_sys_kernal99_face_large_drop2_nb256.hdf5',
