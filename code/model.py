...
class TSM(tf.keras.layers.Layer):
    def call(self, x, n_video, fold_div=3):
        ...
        _, out1 = tf.split(out1, [1, n_video - 1], axis=1)
        ...
        out2, _ = tf.split(out2, [n_video - 1, 1], axis=1)
...
# %%
def MT_CAN_3D(n_frame, nb_filters1, nb_filters2, input_shape, kernel_size=(6, 6, 6), dropout_rate1=0.25,
              dropout_rate2=0.5, pool_size=(4, 4, 4), nb_dense=128):
    diff_input = Input(shape=input_shape)
    rawf_input = Input(shape=input_shape)
